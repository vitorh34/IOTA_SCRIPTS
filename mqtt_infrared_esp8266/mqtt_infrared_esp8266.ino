...
String temp, tempObj, datetime, mac, payload;
...
void setup() {
  ...
  // try to start connect to MLX sensor
  if (!mlx.begin()) {
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    temp = String(mlx.readAmbientTempC());
    tempObj = String(mlx.readObjectTempC()); 
    datetime = DateTime.toISOString();
    mac = WiFi.macAddress().c_str();
    //payload = "{'ts': '%s', 'device': '%s', 'temp': %s, 'tempObject': %s}";
    
    //Serial.println("{'ts': '%s', 'device': '%s', 'temp': %s, 'tempObject': %s}", datetime, mac, temp, tempObj);
    
    //define and publish message to MQTT Broker
    snprintf (msg, MSG_BUFFER_SIZE, "{'ts': '%s', 'device': '%s', 'tempObject': %.2lf, 'temp': %.2lf}", DateTime.toISOString().c_str(), WiFi.macAddress().c_str(), mlx.readObjectTempC(), mlx.readAmbientTempC());
